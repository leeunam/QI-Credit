...
export const validationUtils = {
  ...
  validateFullName: (name: string): { isValid: boolean; message?: string } => {
    ...
    if (name.length > 255) {
      return { isValid: false, message: 'Nome deve ter no máximo 255 caracteres' };
    }
  ...
  validateEmail: (email: string): { isValid: boolean; message?: string } => {
    ...
    if (email.length > 255) {
      return { isValid: false, message: 'Email deve ter no máximo 255 caracteres' };
    }
  ...
  validatePassword: (password: string): { isValid: boolean; message?: string } => {
    ...
    if (password.length > 20) {
      return { isValid: false, message: 'Senha deve ter no máximo 20 caracteres' };
    }
