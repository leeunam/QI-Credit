...
Atualmente, a QI Tech já possui APIs para análise de crédito P2P **entre empresas**, mas ainda não oferece transparência total baseada em blockchain para contratos e contas escrow.  

Nosso diferencial está em usar **smart contracts** para automação de crédito via escrow e **blockchain** para registrar de forma transparente e auditável todos os eventos críticos (hash-only).  

### 4 pilares da plataforma:
- **Smart Contracts:** Automatização do escrow e execução contratual.  
- **Infraestrutura P2P:** Conexão direta entre tomadores e investidores.  
- **Registro em blockchain:** Prova de existência e rastreabilidade (hash-only).  
- **Proof-of-funds:** Garantia de que os fundos estão reservados antes da liberação.  
...
4. Contrato digital é assinado e registrado em blockchain.  
5. Smart contract libera os fundos após aprovação.  
6. Tomador recebe crédito e realiza pagamentos conforme cronograma.  
7. Penalidades aplicadas automaticamente em caso de inadimplência.  
...
- **Blockchain pública/privada** (Scroll, Arbitrum).  
- **Linguagem:** Rust / Solidity para smart contracts.  

### Frontend
- **React.js** para interface do usuário.  
- **Node.js** para integração backend.  
...
- **offers / marketplace_offers** (offer_id, user_id_investor, amount, rate, term, risk_profile, status)  
...
> **TODO:** Incluir DER detalhado com relacionamentos no próximo passo.  
...
**Conceito:**  
- Escrow → fundos ficam em garantia até que condições contratuais sejam atendidas.  
- Smart Contract → substitui o intermediário, executando automaticamente as regras.  
- Blockchain → atua como registro neutro, imutável e auditável.  

**Fluxo resumido:**  
1. Depósito inicial do investidor → smart contract escrow.  
2. Validação do tomador (KYC + score).  
3. Liberação de fundos pelo contrato após aprovação.  
4. Pagamentos periódicos registrados on-chain.  
5. Penalidades automáticas em caso de atraso.  
6. Execução de cláusulas em caso de inadimplência extrema.  

**Snippet de Smart Contract (exemplo simplificado em Solidity):**
